...
public class StockEndpoint {
	...
	// http://localhost:8080/tpv/private/v1.0/stock/producto/id/{identificadorProducto}
	//El path deberia ser producto/{id} pero colosiona con el path que busca productos usando el filtro por lo que aniadimos el "/id"
	@GetMapping("/producto/id/{id}")
	public ResponseEntity<ProductoDTO> getProducto(@PathVariable Long id) {
		try {
			Optional<Producto> producto = controladorStock.obtenerProductoPorId(ProductoId.of(id));
			//Si el Optional viene vacio, el recurso no existe, devuelvo NOT_FOUND
			if (producto.isEmpty()) {
				return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
			} else {
				//Si el Optional tiene valor, devuelvo el producto
				return ResponseEntity.status(HttpStatus.OK).body(productoMapper.toDTO(producto.get()));
			}
			//Si no puedo construir el identificado es que la peticion es erronea BAD_REQUEST
	...
	public ResponseEntity<ProductoDTO> createProducto(@RequestBody ProductoDTO productoDto) {
		...
		CrearProductoCommand cmd = new CrearProductoCommand(productoDto.getDescripcion(), productoDto.getCantidad(),
	...
	public ResponseEntity<ProductoDTO> modificaProducto(@RequestBody ProductoDTO productoDto) {
		...
		try {
			EditarProductoCommand cmd = new EditarProductoCommand(productoDto.getCodigo(), productoDto.getDescripcion(),
			...
			// Si el producto no existe o el id es invalido, devuelvo codigo 400 -> Bad
			// Request
	...
	public ResponseEntity<List<ProductoDTO>> findProducto(@PathVariable String filtro) {
		// OJO: En este punto habria que validar el parametro "filtro" para evitar que
		// metan datos peligrosos
