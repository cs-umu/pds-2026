...
public class ProductoMapper {
	...
	public Producto toModel(ProductoDTO productoDTO) {
		Producto producto = null;
		try {
			producto = new Producto(ProductoId.of(productoDTO.getCodigo()),
	...
	public Producto toModel(ProductoMemory productoMemory) {
		Producto producto = null;
		try {
			producto = new Producto(ProductoId.of(productoMemory.getCodigo()),
					productoMemory.getDescripcion(), productoMemory.getCantidad(), productoMemory.getPrecio());
		} catch (IdentificadorProductoException ex) {
			log.error("Error generando identificador de producto", ex);
		}
		return producto;
	...
	public ProductoMemory toMemoryEntity(Producto producto) {
		return new ProductoMemory(producto.getIdentificador().getCodigoNumerico(), producto.getDescripcion(),
				producto.getCantidad(), producto.getPrecio());
	}
